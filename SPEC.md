...
- [x] **AC-1 (大廳與導航)**：訪問 `/` 正常顯示遊戲大廳，具備典雅中秋夜空視覺與遊戲卡片，點擊可無縫進入 `/games/mooncake` 與 `/games/bbq`。
- [x] **AC-2 (月餅點點樂)**：點擊月餅透過 HTMX 局部更新分數，升級與被動每秒累積正常。
- [x] **AC-3 (烤肉槽位操作)**：玩家可將牛肉串、甜不辣、香腸放上指定烤位，翻面正確切換受熱面。
- [x] **AC-4 (烤肉熟成與刷醬結算)**：每秒正確推進受熱面熟度，雙面熟成判定（完美、夾生、烤焦）與 1.5 倍刷醬加成計算無誤。
- [x] **AC-5 (會話隔離與無刷新)**：所有操作透過 HTMX 局部更新，不同玩家狀態透過 Session 隔離。
- [x] **AC-6 (無 npm / 無 hand-written JS)**：全專案無 `package.json`，不手寫 JS 檔案。
- [x] **AC-7 (TDD 測試全數通過)**：執行 `python -m unittest` 時，所有遊戲引擎與 API 測試 100% 通過 (25/25 Pass)。
